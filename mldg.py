...
def train_target(args):
    ...
    elif args.data in ['seizure'] and args.method == 'MLDG':
    ...
    patience = 10
    model_path_final = '/mnt/data2/zwwang/TASA_code/Feature_based/models/'
    ...
    meta_modelF = l2l.algorithms.MAML(netF, lr=args.lr)
    meta_optF = optim.Adam(meta_modelF.parameters(), lr=args.lr)

    meta_modelC = l2l.algorithms.MAML(netC, lr=args.lr)
    meta_optC = optim.Adam(meta_modelC.parameters(), lr=args.lr)
    ...
    for iter_num in range(max_iter):  # max_iter = max_epoch * num of double minimatches
        ...
        for i in range(int(np.sum(batch_nums) / 2)):
            ...
            inner_netF = meta_modelF.clone()
            inner_netC = meta_modelC.clone()
            _, out = inner_netC(inner_netF(xi))
            inner_loss = tr.nn.functional.cross_entropy(out, yi)
            inner_netF.adapt(inner_loss)
            inner_netC.adapt(inner_loss)

            _, predictions = inner_netC(inner_netF(xj))
...
if __name__ == '__main__':
    ...
    for seed in seed_arr:
        ...
        for idt in range(n_subject):
            ...
            args.rate = 0.9
            args.lamda1 = 0.1  # 0.1,1 is the best group on validation set
            args.lamda2 = 1
            ...
            args.result_dir = '/mnt/data2/zwwang/TASA_code/Feature_based/result_subject/CHSZ/'
